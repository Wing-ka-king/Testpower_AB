...
class Parser(object):
    ...
    def data_assign(self, bus, recv_bus):
        ...
        for i in range(self.n_param):
            if self.headers[i] == 'StoreNo':  # ID 100 byte size = 3
                ...
                bus.send(Message(arbitration_id=100, data=to_can_msg))
                ...
                bus.send(Message(arbitration_id=101, data=to_can_msg))
                ...
                bus.send(Message(arbitration_id=102, data=to_can_msg))
                ...
                hex_data = '0x{0:0{1}X}'.format(int(self.row_data[i], 10), byte_size * 2)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=103, data=to_can_msg))
                ...
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=104, data=to_can_msg))
                ...
                # hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=105, data=to_can_msg))
                ...
                hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=106, data=to_can_msg))
                ...
                hex_data = '0x{0:0{1}X}'.format(self.row_data[i], byte_size * 2)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=107, data=to_can_msg))
                ...
                # hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=108, data=to_can_msg))
                ...
                hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=109, data=to_can_msg))

            elif self.headers[i] == 'Urange':  # ID 110 byte size = 2
                to_can_msg = []
                byte_size = 2
                if self.row_data[i] == 'Auto':
                    bus.send(Message(arbitration_id=110, data=[0xff, 0xff]))
                    pass
                else:
                    ...
                    hex_data = signedhex(self.row_data[i], byte_size * 8)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=110, data=to_can_msg))
                    pass
                ...
                hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=111, data=to_can_msg))
                ...
                hex_data = '0x{0:0{1}X}'.format(self.row_data[i], byte_size * 2)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=112, data=to_can_msg))
                ...
                hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=113, data=to_can_msg))
                ...
                hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=114, data=to_can_msg))
                ...
                hex_data = '0x{0:0{1}X}'.format(self.row_data[i], byte_size * 2)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=115, data=to_can_msg))
                ...
                hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=116, data=to_can_msg))

            elif self.headers[i] == 'Irange':  # ID 117 byte size = 2
                to_can_msg = []
                byte_size = 2
                if self.row_data[i] == 'Auto':
                    bus.send(Message(arbitration_id=117, data=[0xff, 0xff]))
                    pass
                else:
                    self.row_data[i] = int(float(self.row_data[i]) * 10)
                    # hex_data = '0x{0:0{1}X}'.format(self.row_data[i], byte_size * 2)
                    hex_data = signedhex(self.row_data[i], byte_size * 8)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=117, data=to_can_msg))
                    pass

            elif self.headers[i] == 'P-1':  # ID 118, byte size = 3
                to_can_msg = []
                byte_size = 3
                # bus.send("{:.2f}".format(float(self.row_data[i])))
                self.row_data[i] = int(float(self.row_data[i]) * 100)  # upto 2 decimal places
                # hex_data = '0x{0:0{1}X}'.format(self.row_data[i], byte_size * 2)
                hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=118, data=to_can_msg))
                ...
                hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=119, data=to_can_msg))
                ...
                hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=120, data=to_can_msg))

            elif self.headers[i] == 'PF-1':  # ID 121, byte size = 3
                to_can_msg = []
                byte_size = 3
                if self.row_data[i] == 'Error    ':
                    bus.send(Message(arbitration_id=121, data=[0xa0, 0xa0, 0xa0]))
                    pass
                else:
                    self.row_data[i] = int(float(self.row_data[i]) * 1000)
                    # hex_data = '0x{0:0{1}X}'.format(self.row_data[i], byte_size * 2)
                    hex_data = signedhex(self.row_data[i], byte_size * 8)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=121, data=to_can_msg))
                    pass
                ...
                byte_size = 3
                if self.row_data[i] == 'Error    ':
                    bus.send(Message(arbitration_id=122, data=[0xa0, 0xa0, 0xa0]))
                    pass
                else:
                    self.row_data[i] = int(float(self.row_data[i]) * 1000)
                    # hex_data = '0x{0:0{1}X}'.format(self.row_data[i], byte_size * 2)
                    hex_data = signedhex(self.row_data[i], byte_size * 8)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=122, data=to_can_msg))
                    pass
                ...
                byte_size = 3
                if self.row_data[i] == 'Error    ':
                    bus.send(Message(arbitration_id=123, data=[0xa0, 0xa0, 0xa0]))
                else:
                    self.row_data[i] = int(float(self.row_data[i]) * 1000)
                    hex_data = '0x{0:0{1}X}'.format(self.row_data[i], byte_size * 2)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=123, data=to_can_msg))

            elif self.headers[i] == 'FreqI-1':  # ID 124, byte size = 3
                to_can_msg = []
                byte_size = 3
                if self.row_data[i] == 'Error    ':
                    bus.send(Message(arbitration_id=124, data=[0xa0, 0xa0, 0xa0]))
                    pass
                else:
                    self.row_data[i] = int(float(self.row_data[i]) * 1000)
                    hex_data = '0x{0:0{1}X}'.format(self.row_data[i], byte_size * 2)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=124, data=to_can_msg))
                ...
                hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=125, data=to_can_msg))
                ...
                hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=126, data=to_can_msg))
                ...
                hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=127, data=to_can_msg))
                ...
                hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=128, data=to_can_msg))
                ...
                hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=129, data=to_can_msg))
                ...
                hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=130, data=to_can_msg))
                ...
                hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=131, data=to_can_msg))

            elif self.headers[i] == 'TIME':  # ID 132 byte size = 4
                to_can_msg = []
                byte_size = 4
                if self.row_data[i] == 'NAN':
                    bus.send(Message(arbitration_id=132, data=[0x0a, 0x0a, 0x0a, 0x0a]))
                    pass
                else:
                    ...
                    # hex_data = signedhex(self.row_data[i], byte_size * 8)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=132, data=to_can_msg))
                    pass
                ...
                byte_size = 3
                if self.row_data[i] == 'NAN':
                    bus.send(Message(arbitration_id=133, data=[0x0a, 0x0a, 0x0a]))
                    pass
                else:
                    ...
                    hex_data = signedhex(self.row_data[i], byte_size * 8)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=133, data=to_can_msg))
                    pass
                ...
                byte_size = 3
                if self.row_data[i] == 'NAN':
                    bus.send(Message(arbitration_id=134, data=[0x0a, 0x0a, 0x0a]))
                    pass
                else:
                    self.row_data[i] = int(float(self.row_data[i]) * 10E8)
                    # hex_data = '0x{0:0{1}X}'.format(self.row_data[i], byte_size * 2)
                    hex_data = signedhex(self.row_data[i], byte_size * 8)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=134, data=to_can_msg))
                    pass
                ...
                byte_size = 3
                if self.row_data[i] == 'NAN':
                    bus.send(Message(arbitration_id=135, data=[0x0a, 0x0a, 0x0a]))
                    pass
                else:
                    self.row_data[i] = int(float(self.row_data[i]) * 10E8)
                    # hex_data = '0x{0:0{1}X}'.format(self.row_data[i], byte_size * 2)
                    hex_data = signedhex(self.row_data[i], byte_size * 8)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=135, data=to_can_msg))
                    pass
                ...
                byte_size = 3
                if self.row_data[i] == 'NAN':
                    bus.send(Message(arbitration_id=136, data=[0x0a, 0x0a, 0x0a]))
                    pass
                else:
                    ...
                    hex_data = signedhex(self.row_data[i], byte_size * 8)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=136, data=to_can_msg))
                    pass
                ...
                if self.row_data[i] == 'NAN':
                    ...
                    hex_data = signedhex(self.row_data[i], byte_size * 8)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=139, data=to_can_msg))
                    pass
                ...
                if self.row_data[i] == 'NAN':
                    ...
                    hex_data = signedhex(self.row_data[i], byte_size * 8)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=140, data=to_can_msg))
                    pass
                ...
                if self.row_data[i] == 'NAN':
                    ...
                    hex_data = signedhex(self.row_data[i], byte_size * 8)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=141, data=to_can_msg))
                    pass
                ...
                byte_size = 3
                if self.row_data[i] == 'NAN':
                    bus.send(Message(arbitration_id=142, data=[0x0a, 0x0a, 0x0a]))
                    pass
                else:
                    self.row_data[i] = int(float(self.row_data[i]) * 1000)
                    # hex_data = '0x{0:0{1}X}'.format(self.row_data[i], byte_size * 2)
                    hex_data = signedhex(self.row_data[i], byte_size * 8)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=142, data=to_can_msg))
                    pass
                ...
                byte_size = 3
                if self.row_data[i] == 'NAN':
                    bus.send(Message(arbitration_id=143, data=[0x0a, 0x0a, 0x0a]))
                    pass
                else:
                    self.row_data[i] = int(float(self.row_data[i]) * 1000)
                    # hex_data = '0x{0:0{1}X}'.format(self.row_data[i], byte_size * 2)
                    hex_data = signedhex(self.row_data[i], byte_size * 8)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=143, data=to_can_msg))
                    pass
                ...
                byte_size = 3
                if self.row_data[i] == 'NAN':
                    bus.send(Message(arbitration_id=144, data=[0x0a, 0x0a, 0x0a]))
                    pass
                else:
                    self.row_data[i] = int(float(self.row_data[i]) * 1000)
                    # hex_data = '0x{0:0{1}X}'.format(self.row_data[i], byte_size * 2)
                    hex_data = signedhex(self.row_data[i], byte_size * 8)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=144, data=to_can_msg))
                    pass
                ...
                byte_size = 3
                if self.row_data[i] == 'Error    ':
                    bus.send(Message(arbitration_id=145, data=[0xa0, 0xa0, 0xa0]))
                    pass
                elif self.row_data[i] == 'NAN':
                    bus.send(Message(arbitration_id=145, data=[0x0a, 0x0a, 0x0a]))
                    pass
                else:
                    ...
                    hex_data = signedhex(self.row_data[i], byte_size * 8)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=145, data=to_can_msg))
                    pass
                ...
                byte_size = 3
                if self.row_data[i] == 'Error    ':
                    bus.send(Message(arbitration_id=146, data=[0xa0, 0xa0, 0xa0]))
                    pass
                elif self.row_data[i] == 'NAN':
                    bus.send(Message(arbitration_id=146, data=[0x0a, 0x0a, 0x0a]))
                    pass
                else:
                    self.row_data[i] = int(float(self.row_data[i]) * 10E4)
                    # hex_data = '0x{0:0{1}X}'.format(self.row_data[i], byte_size * 2)
                    hex_data = signedhex(self.row_data[i], byte_size * 8)
                    to_can_msg = message_array(hex_data)
                    bus.send(Message(arbitration_id=146, data=to_can_msg))
                    pass
                ...
                # hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=147, data=to_can_msg))
                ...
                # hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=148, data=to_can_msg))
                ...
                # hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=149, data=to_can_msg))
                ...
                # hex_data = signedhex(self.row_data[i], byte_size * 8)
                to_can_msg = message_array(hex_data)
                bus.send(Message(arbitration_id=150, data=to_can_msg))
...
for row in csv_reader:
    ...
    csv_data.data_assign(bus_send, bus_recv)
    # break
